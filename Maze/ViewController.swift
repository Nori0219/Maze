...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        let cellWidth = screenSize.width / CGFloat(maze[0].count)//横画面のサイズを横のマスの数（6マス）で割る
        let cellHeight = screenSize.height / CGFloat(maze.count)//縦画面のサイズを縦のマス（10マス）で割る
        
        let cellOffsetX  = cellWidth / 2 //マスの左上とマスの中心のx座標の差
    ...
    //迷路の一マスとなるUIViewを作るメソッド　どの要素のマス、マスの大きさから位置・大きさを設定して生成する
    ...
    func startAccelerometer() {
        //加速度を取得
        let handler: CMAccelerometerHandler = { (CMAccelerometerDate: CMAccelerometerData?, error:
            Error?) -> Void in
            self.speedX += CMAccelerometerDate!.acceleration.x
            self.speedY += CMAccelerometerDate!.acceleration.y
            //プレイヤーの中心位置を設定
            var posX = self.playerView.center.x + (CGFloat(self.speedX) / 3)
            var posY = self.playerView.center.y + (CGFloat(self.speedY) / 3)
            
            //画面外へプレイヤーがはみ出しそうになったらposを修正
            ...
            for walllRect in self.wallReactArray {
                if walllRect.intersects(self.playerView.frame) {
